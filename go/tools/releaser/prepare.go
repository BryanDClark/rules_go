...
func runPrepare(ctx context.Context, stderr io.Writer, args []string) error {
	...
	release, err := gh.getReleaseByTagIncludingDraft(ctx, "BryanDClark", "rules_go", version)
	...
	arcGHURLWithoutScheme := fmt.Sprintf("github.com/BryanDClark/rules_go/releases/download/%[1]s/rules_go-%[1]s.zip", version)
	if uploadToMirror {
	...
	if release == nil {
		...
		if release, _, err = gh.Repositories.CreateRelease(ctx, "BryanDClark", "rules_go", release); err != nil {
			return err
		}
	} else {
		fmt.Fprintf(stderr, "updating release...\n")
		release.Body = &rnotesStr
		if release, _, err = gh.Repositories.EditRelease(ctx, "BryanDClark", "rules_go", release.GetID(), release); err != nil {
			return err
		}
		for _, asset := range release.Assets {
			if _, err := gh.Repositories.DeleteReleaseAsset(ctx, "BryanDClark", "rules_go", asset.GetID()); err != nil {
				return err
			}
		}
	}
	...
	if _, _, err := gh.Repositories.UploadReleaseAsset(ctx, "BryanDClark", "rules_go", release.GetID(), uploadOpts, arcFile); err != nil {
		return err
	}
